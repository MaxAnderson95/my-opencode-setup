...
Set `TYPESAFE_API_KEY` in the TUI process environment or `~/.env_private` on the machine running the TUI. The plugin parses that file as dotenv data on each invocation when the process environment lacks the key. It sends one request to `https://api.typesafe.ai/v1/systemone` using `jev-latest`.
